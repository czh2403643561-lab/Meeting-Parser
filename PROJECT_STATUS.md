...
- 本地工具桌面入口框架已实现：新增独立 `MeetingParserTool.exe`，包含主窗口、功能入口占位区和状态区；扩展可通过 Native Messaging 打开工具。
- 本地工具已增加“MP4 转 MP3”：支持多文件选择、串行转换、默认同目录 MP3 输出、可选输出目录、实时进度、任务统计和折叠日志；输出目录保存到 `%LOCALAPPDATA%\MeetingParser\config.json`，FFmpeg/ffprobe 随工具打包。
- 项目根目录已增加 `Open-MeetingParserTool.bat`，可优先启动已安装或开发构建的 `MeetingParserTool.exe`，不依赖 Chrome、PowerShell 或 Python。
...
- 本地工具 GUI 已通过启动测试；两个测试 MP4 已串行转换为 MP3，原 MP4 保留，无效 MP4 能显示失败原因。
- 新 Setup 已成功编译，`MeetingParserTool.exe` 内含 FFmpeg、ffprobe 及其 DLL 依赖。
- GitHub Actions Setup 构建流程已补充 FFmpeg 构建依赖安装，发布链路可继续生成带转换能力的工具。
- 独立工具输出目录配置、指定目录转换和配置恢复测试已通过；新构建的 `MeetingParserTool.exe` 已实际启动验证。
...
- 本地工具仍待在安装包更新后进行一次真实安装和扩展点击启动验收；输出目录按钮和重启恢复仍待用户真实桌面验收。
- MP4 转 MP3 仍待使用用户真实录制的多个 MP4 文件进行桌面工具验收。
...
- 构建并安装新的 Setup 后，点击扩展中的“打开本地工具”，确认 `MeetingParserTool.exe` 能正常启动；继续保持 MP4、批量下载和逐字稿链路不变。
- 使用多个真实 MP4 文件验收 MP3 输出、原文件保留、指定输出目录、打开文件夹、重启恢复和失败日志。
