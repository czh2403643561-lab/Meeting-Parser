...
- Side Panel 的折叠区域已提供“提取逐字稿”入口和解析状态；content script 已优先读取 `pid-*-content`，自动滚动虚拟列表、按段落顺序去重合并，并恢复用户原滚动位置。
...
- 刷新本地扩展后，需要在真实课程页面确认自动滚动采集能得到完整正文、最终字数明显超过原先约 2000 字，并验收复制全文和 TXT 导出结果。

## 下一步

- 在 Chrome 扩展管理页刷新本地扩展后，使用真实腾讯会议录制页点击“页面诊断/提取逐字稿”，确认完整正文、复制全文和 TXT 导出；再继续固定 LocalAppData、单条下载和“重新检测”验收。
