...
function Build-CompanionExecutable {
  ...
  & $Python -3 -m PyInstaller --noconfirm --clean --onefile --windowed --name $Name `
    --distpath $distRoot --workpath $workRoot --specpath $workRoot $Source
...
Build-CompanionExecutable -Name "MeetingParserNativeHost" -Source (Join-Path $projectRoot "companion\native_host.py")
