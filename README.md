...
插件会在需要时通过浏览器 Native Messaging 静默启动本地下载组件；组件只在浏览器连接存在时运行，断开后自动退出。批量下载运行时仅阻止系统因空闲进入睡眠，不会保持屏幕常亮。
...
开发调试仍可直接运行 Python 下载器：

```powershell
py local_downloader.py
```

它仅监听 `127.0.0.1:8765`，下载仍保存到用户 Downloads 文件夹；正式安装包不使用这个本地 HTTP 服务。这个命令仅用于开发调试，不是普通用户的日常步骤。
